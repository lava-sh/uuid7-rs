...
fn extract_random_bits(
    ...
    if has_ts && has_nanos {
        ...
        *tail62 = w1rand() & MASK62;
        return 0;
    }
    if has_ts || has_nanos {
        let counter = w1rand() & MASK42;
        *rand_a = (counter >> 30) as u16;
        *tail62 = ((counter & MASK30) << 32) | u64::from(w1rand() as u32);
        return 0;
    }
    1
}
...
fn extract_random_bits_secure(
    ...
    if has_ts || has_nanos {
        let (c, r) = match (rnd_u64_secure(), rnd_u64_secure()) {
            (Ok(a), Ok(b)) => (a, b),
            _ => return -1,
        };
